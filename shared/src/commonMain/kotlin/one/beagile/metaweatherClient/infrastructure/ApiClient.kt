package one.beagile.metaweatherClient.infrastructure
...
import one.beagile.metaweatherClient.apis.*
import one.beagile.metaweatherClient.models.*
import one.beagile.metaweatherClient.auth.*

data class Config(val userAgent: String)

fun makeClient(json: Json): HttpClient {
    return HttpClient() {
        install(JsonFeature) {
            this.serializer = KotlinxSerializer(json).ignoreOutgoingContent()
        }
    }
}
...
open class ApiClient(
    ...
    private val client: HttpClient = makeClient(json)
